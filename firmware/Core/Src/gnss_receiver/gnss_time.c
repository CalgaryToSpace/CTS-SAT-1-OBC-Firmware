...
uint8_t GNSS_set_obc_time_based_on_gnss_time() {
    ...
    // Function to parse AND FORMAT the GNSS response (add unit test) -> will return timestamp value
    const uint64_t formatted_time = GNSS_format_and_convert_to_unix_epoch(response_str);

    // TODO: Uncomment the following call `TIM_set_current_unix_epoch_time_ms` after GNSS_format_and_convert_to_unix_epoch is finished
