...
// TODO: look into the `LFS_F_INLINE` macro to increase the maximum number of
// files we can have
...
struct lfs_config cfg = {
    ...
    .block_size =
        FLASH_CHIP_BLOCK_SIZE_BYTES, // FIXME: Clarify block Size 256KiB or 1KiB
    .block_count = (FLASH_CHIP_SIZE_BYTES / FLASH_CHIP_BLOCK_SIZE_BYTES),
    .block_cycles =
        100, // TODO: ASK ABOUT THIS (HOW FREQUENT ARE WE USING THE MODULE),
    .cache_size = 512,
    .lookahead_size = 16,
    .compact_thresh =
        -1, // Defaults to ~88% block_size when zero (lfs.h, line 232)
 ...
 * @param None
 * @retval 0 on success, 1 if LFS is already mounted, negative LFS error codes
 * on failure
 */
int8_t LFS_mount() {
 ...
 * @param None
 * @retval 0 on success, 1 if LFS is already unmounted, negative LFS error codes
 * on failure
 */
int8_t LFS_unmount() {
  if (!LFS_is_lfs_mounted) {
 ...
 * @param root_directory Pointer to cstring holding the root directory to open
 * and read
 * @retval 0 on success, 1 if LFS is unmounted, negative LFS error codes on
 * failure
 */
int8_t LFS_list_directory(char *root_directory) {
  ...
  // result is positive on success, 0 at the end of directory, or  negative on
  // failure.
 ...
 * failure
 */
int8_t LFS_write_file(char *file_name, uint8_t *write_buffer,
  ...
  lfs_file_t file;
  const int8_t open_result =
      lfs_file_open(&lfs, &file, file_name, LFS_O_WRONLY | LFS_O_CREAT);
  ...
  // Write data to file
  const int8_t write_result =
      lfs_file_write(&lfs, &file, write_buffer, write_buffer_len);
  if (write_result < 0) {
  ...
  // Close the File, the storage is not updated until the file is closed
  // successfully
 ...
 */
int8_t LFS_read_file(char *file_name, uint8_t *read_buffer,
                     uint32_t read_buffer_len) {
  lfs_file_t file;
  const int8_t open_result =
      lfs_file_open(&lfs, &file, file_name, LFS_O_RDONLY);
  if (open_result < 0) {
  ...
  const int8_t read_result =
      lfs_file_read(&lfs, &file, read_buffer, read_buffer_len);
  if (read_result < 0) {
