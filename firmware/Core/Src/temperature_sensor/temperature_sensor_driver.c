...
uint8_t read_temperature(int32_t *result)
{
    ...
    *result = TEMP_SENSOR_convert_raw_to_deg_c(buf, TEMP_SENSOR_precision_coefficient, TEMP_SENSOR_precision_insignificant_bits);
                                ...
                                Temperature_Sensor_Data_Precision_Insignificant_Bytes_t TEMP_SENSOR_data_precision_insignificant_bits)
{
    ...
    int32_t result = temp * TEMP_SENSOR_precision_scaling_factor;
...
uint8_t TEMP_SENSOR__set_temp_precision(uint8_t arg_precision, uint8_t did_user_set_precision)
{
    ...
    {
        case 9:
            TEMP_SENSOR_precision_scaling_factor = 10;
            TEMP_SENSOR_precision_insignificant_bits = TEMP_SENSOR_NINE_BIT_PRECISION_INSIGNIFICANT_BYTES;
            TEMP_SENSOR_precision_coefficient = TEMP_SENSOR_nine_bit_precision_coefficient;
            precision_to_write = 0;
            break;
        case 10:
            TEMP_SENSOR_precision_scaling_factor = 100;
            TEMP_SENSOR_precision_insignificant_bits = TEMP_SENSOR_TEN_BIT_PRECISION_INSIGNIFICANT_BYTES;
            TEMP_SENSOR_precision_coefficient = TEMP_SENSOR_ten_bit_precision_coefficient;
            precision_to_write = 0x01 << 5;
            break;
        case 11: 
            TEMP_SENSOR_precision_scaling_factor = 1000;
            TEMP_SENSOR_precision_insignificant_bits = TEMP_SENSOR_ELEVEN_BIT_PRECISION_INSIGNIFICANT_BYTES;
            TEMP_SENSOR_precision_coefficient = TEMP_SENSOR_eleven_bit_precision_coefficient;
            precision_to_write = 0x02 << 5;
            break;
        case 12: 
            TEMP_SENSOR_precision_scaling_factor = 10000;
            TEMP_SENSOR_precision_insignificant_bits = TEMP_SENSOR_TWELVE_BIT_PRECISION_INSIGNIFICANT_BYTES;
            TEMP_SENSOR_precision_coefficient = TEMP_SENSOR_twelve_bit_precision_coefficient;
            precision_to_write = 0x03 << 5;
            break;
        default:
            return 2;
    }
    ...
    status = HAL_I2C_Mem_Write(&hi2c1, TEMP_SENSOR_device_addr << 1, TEMP_SENSOR_config_register_addr, 1, &precision_to_write, 1, HAL_MAX_DELAY);
    if (status != HAL_OK)
    {
        // write error
        return 1;
    ...
    TEMP_SENSOR_custom_precision_set = did_user_set_precision;
