...
static int32_t schedule_csp_packet_data_for_tcmd_execution(uint8_t * packet, size_t packet_size) {
	char* packet_data = (char*) strip_csp_packet( COMMS_rx_buffer, packet_size);
...
{	
	...
	schedule_csp_packet_data_for_tcmd_execution(COMMS_rx_buffer, rx_count);
...
{
	rx_count++;
	if (rx_count < COMMS_RX_BUFF_SIZE-1) {
