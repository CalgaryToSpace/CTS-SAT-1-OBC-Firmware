...
int rx_count = 0;
#define CSP_RX_BUFF_SIZE 255
uint8_t csp_rx_buffer[CSP_RX_BUFF_SIZE];
...
static int32_t schedule_csp_packet_data_for_tcmd_execution(uint8_t * packet, size_t packet_size) {
	char* packet_data = (char*) strip_csp_packet( csp_rx_buffer, packet_size);
...
{	
	csp_rx_buffer[CSP_RX_BUFF_SIZE -1] = '\0'; // enusure the buffer is null terminated

	for (int i = 0; i < rx_count; i++) {
	LOG_message(
		LOG_SYSTEM_UHF_RADIO, LOG_SEVERITY_NORMAL, LOG_SINK_ALL,
		"I2C listen complete, data: %x", csp_rx_buffer[i]);
	}
	schedule_csp_packet_data_for_tcmd_execution(csp_rx_buffer, rx_count);
...
void HAL_I2C_AddrCallback(I2C_HandleTypeDef *hi2c, uint8_t TransferDirection, uint16_t AddrMatchCode)
{
	if (TransferDirection == I2C_DIRECTION_TRANSMIT)  // if the master wants to transmit the data
	{
		...
		HAL_I2C_Slave_Seq_Receive_IT(hi2c, csp_rx_buffer + rx_count, 1, I2C_FIRST_FRAME);
		LOG_message(
			LOG_SYSTEM_UHF_RADIO, LOG_SEVERITY_NORMAL, LOG_SINK_ALL,
			"first byte received"
		);
...
{
	rx_count++;
	if (rx_count < CSP_RX_BUFF_SIZE-1) {
		// The I2C_NEXT_FRAME implies that we are expecting the next byte of the same transfer
		HAL_I2C_Slave_Seq_Receive_IT(hi2c, csp_rx_buffer + rx_count, 1, I2C_NEXT_FRAME);
	}
	else {
		...
		HAL_I2C_Slave_Seq_Receive_IT(hi2c, csp_rx_buffer + rx_count, 1, I2C_LAST_FRAME);
	}
}
