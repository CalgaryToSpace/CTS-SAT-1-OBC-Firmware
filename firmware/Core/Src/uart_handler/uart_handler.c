...
const uint16_t UART_mpi_buffer_len = 50; // extern  // Max possible MPI response buffer size allocated to 50 bytes (Considering for the telecommand echo response, NOT science data.
volatile uint8_t UART_mpi_buffer[50]; // extern
...
volatile uint8_t UART_lora_buffer_last_rx_byte = 0; // extern
