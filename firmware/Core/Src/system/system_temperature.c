...
uint8_t SYS_TEMP_get_raw_thermal_info(SYS_TEMP_raw_thermal_info_t* result, uint8_t* error_ret){
    ...
    uint8_t ANT_A_status = ANT_CMD_measure_temp(ANT_I2C_BUS_A_MCU_A, &ANT_raw_temp_A);
    if (ANT_A_status!=0) {
    ...
    uint8_t ANT_B_status = ANT_CMD_measure_temp(ANT_I2C_BUS_B_MCU_B, &ANT_raw_temp_B);
    if (ANT_B_status!=0){
    ...
    uint8_t pcu_status = EPS_CMD_get_pcu_housekeeping_data_run_avg(&pcu_data);
    if (pcu_status!=0){
    ...
    uint8_t pbu_status = EPS_CMD_get_pbu_housekeeping_data_eng(&pbu_data);
    if (pbu_status!=0){
...
uint8_t SYS_TEMP_pack_to_system_thermal_info(SYS_TEMP_raw_thermal_info_t* input, SYS_TEMP_thermal_info_t* result, uint8_t error_ret) {
    ...
    result->system_eps_battery_heater_status_bit = ((error_ret >> 3) & 1) ? 9999 : (bp_status_bitfield >> 12) & 1;
