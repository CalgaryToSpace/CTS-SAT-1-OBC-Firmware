...
/// @brief Converts a datetime in MS-DOS format to conventional datetime formatting (used in the ADCS SD Card index file). 
/// @note Code modified from that provided by ADCS Firmware Reference Manual Section 7.2.2
/// @param[in] msdostime the MS-DOS time input (4 bytes)
/// @return -1 if there was an error, otherwise a time_t version of the input time in Unix seconds
time_t ADCS_convert_msdos_to_unix_time(uint32_t msdostime) {
    /*
        MS-DOS Date & Time (4 bytes)
        The lower word determines the time, the upper word the date. Used by several DOSs
        function calls and by all FAT file systems.
        Bits Contents
        0-4 Second divided by 2
        5-10 Minute (0-59)
        11-15 Hour (0-23 on a 24-hour clock)
        16-20 Day of the month (1-31)s
        21-24 Month (1 = January, 2 = February, etc.)
        25-31 Year offset from 1980
    */

    uint8_t seconds = (msdostime & 0x1f) << 1; // 5bits - 32
    uint8_t minutes = (msdostime >> 5) & 0x3f; // 6bits - 64
    uint8_t hour = (msdostime >> 11) & 0x1f; // 5bits - 32
    uint8_t day = (msdostime >> 16) & 0x1f; // 5bits - 32
    uint8_t month = (msdostime >> 21) & 0x0f; // 4bits - 16
    uint16_t year = (msdostime >> 25) + 1980; // 7bits - 128 (1980 to 21..)

    /* below this line is my own code */
    ...
    time_struct.tm_year = year - 1900; // Provided code counts from 1980, but <time.h> requires counting from 1900

    time_t status = mktime(&time_struct);

    return status;
} // TODO: write test for this... assuming I can actually find a datetimestamp in MSDOS anywhere...