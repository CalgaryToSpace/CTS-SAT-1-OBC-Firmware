...
/// @param new_baud_rate The new baud rate to set the camera to
...
/// @brief Sets up the camera by powering on and changing the baudrate to 2400.
...
uint8_t CAM_test() {
    ...
    // Seems that the test sentence response should be 202 bytes.
    uint8_t UART_test_str[300];
    UART_test_str[0] = 0; // Preset in order to check if the response is empty.

    const HAL_StatusTypeDef rx_status = HAL_UART_Receive(
        UART_camera_port_handle, UART_test_str, sizeof(UART_test_str), 5000
    );
    if (rx_status != HAL_OK) {
        LOG_message(
            LOG_SYSTEM_BOOM, LOG_SEVERITY_ERROR, LOG_SINK_ALL,
    ...
    // FIXME: Automatically validate the `UART_test_str` response further (check total length).
