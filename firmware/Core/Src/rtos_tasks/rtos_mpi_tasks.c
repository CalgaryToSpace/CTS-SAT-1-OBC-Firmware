...
void TASK_service_write_mpi_data(void *argument) {
    ...
    while(1) {
        ...
        if (MPI_buffer_state == MPI_MEMORY_WRITE_STATUS_PENDING && MPI_receive_prepared) {
            ...
            // Write data to file
            const lfs_ssize_t write_result = lfs_file_write(&LFS_filesystem, &MPI_science_data_file_pointer, MPI_active_data_median_buffer, MPI_active_data_median_buffer_len);
            if (write_result < 0)
            {
                LOG_message(LOG_SYSTEM_MPI, LOG_SEVERITY_WARNING, LOG_all_sinks_except(LOG_SINK_FILE), "Error writing to file: %ld", write_result);
            } else {
                LOG_message(LOG_SYSTEM_MPI, LOG_SEVERITY_NORMAL, LOG_all_sinks_except(LOG_SINK_FILE), "Succesfully Wrote 4096 bytes");
