	...
	while (1) {
		...
		if (1) {
			...
			if (latest_tcmd_len > 0) {
                LOG_message(LOG_SYSTEM_OBC, LOG_SEVERITY_NORMAL, 
                        LOG_CHANNEL_ALL, 
                        "Received telecommand (len=%lu): '%s'", 
                        latest_tcmd_len, 
                        (char *)latest_tcmd
                );
			}
			...
			if (!TCMD_check_starts_with_device_id((char *)latest_tcmd, latest_tcmd_len)) {
                LOG_message(LOG_SYSTEM_TELECOMMAND, LOG_SEVERITY_ERROR, LOG_CHANNEL_ALL, "Telecommand does not start with the correct prefix");
				continue;
			}
			...
			if (tcmd_idx < 0) {
                LOG_message(LOG_SYSTEM_TELECOMMAND, LOG_SEVERITY_ERROR, LOG_CHANNEL_ALL, "Telecommand not found in the list");
				continue;
			}

			// get the telecommand definition
			TCMD_TelecommandDefinition_t tcmd_def = TCMD_telecommand_definitions[tcmd_idx];
            LOG_message(LOG_SYSTEM_TELECOMMAND, LOG_SEVERITY_NORMAL, LOG_CHANNEL_ALL, "==== Received telecommand %s ====", tcmd_def.tcmd_name);
			...
			if (latest_tcmd_len < start_of_args_idx + 1) {
                LOG_message(LOG_SYSTEM_TELECOMMAND, LOG_SEVERITY_ERROR, LOG_CHANNEL_ALL, "Missing parenthesis");
				continue;
			}
			if (latest_tcmd[start_of_args_idx] != '(') {
                LOG_message(LOG_SYSTEM_TELECOMMAND, LOG_SEVERITY_ERROR, LOG_CHANNEL_ALL, "Missing opening parenthesis");
				continue;
			}
			...
			if (end_of_args_idx < 0) {
                LOG_message(LOG_SYSTEM_TELECOMMAND, LOG_SEVERITY_ERROR, LOG_CHANNEL_ALL, "Missing closing parenthesis");
				continue;
			}

			// TODO: maybe log/print args too, except long sequences of bytes (like
            // firmware bytes)
            ...
            LOG_message(LOG_SYSTEM_TELECOMMAND, LOG_SEVERITY_NORMAL, LOG_CHANNEL_ALL, "==== Got telecommand response (duration=%d ms, err=%ud %s) ====", tcmd_exec_duration_ms, tcmd_result, tcmd_result != 0 ? "!!!! ERROR !!!!" : "");
            ...
            LOG_message(LOG_SYSTEM_TELECOMMAND, LOG_SEVERITY_ERROR, LOG_CHANNEL_ALL, "Only partial telecommand received. Buffer length: %ud bytes filled", latest_tcmd_len);
		}
