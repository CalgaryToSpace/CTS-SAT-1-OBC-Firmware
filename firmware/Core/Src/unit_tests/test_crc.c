...
uint8_t TEST_EXEC__test_crc_empty()
{
    ...
    uint8_t data[] = {0};
    // Compute CRC with null data
    uint32_t crc = GEN_crc32_checksum(&hcrc1, data, 1);

    // Assert the CRC remains at its initial value
...
uint8_t TEST_EXEC__test_crc_half()
{
    ...
    */
    ...
    uint32_t expected_crc = 0x6C12E412;  // CRC from online calculator

    // Compute CRC with half data
...
uint8_t TEST_EXEC__test_crc_random()
{
    ...
    */
    ...
    uint32_t expected_crc = 0xC1F6048C;  // CRC from online calculator

    // Compute CRC with non divisible by 2 data size
...
{
    char* data ="#@?/`')(";
    uint32_t expected_crc = 0x72D7BC15;  // CRC from online calculator

    // Compute CRC with special characters as data
...
uint8_t TEST_EXEC__test_crc_odd()
{
    ...
    uint32_t expected_crc = 0x585C8EA2;  // CRC from online calculator
