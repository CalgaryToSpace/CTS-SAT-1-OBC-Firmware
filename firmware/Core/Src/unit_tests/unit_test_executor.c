...
uint8_t TEST_run_all_unit_tests_and_log(char log_buffer[], uint16_t log_buffer_size) {
    ...
    for (uint8_t test_num = 0; test_num < TEST_definitions_count; test_num++) {
        ...
        LOG_message(LOG_SYSTEM_UNIT_TEST, LOG_SEVERITY_NORMAL, LOG_CHANNEL_ALL, "Test #%03d: %s (%s > %s)",
            test_num, 
            (result == 0) ? "PASS ✅" : "FAIL ❌",
    ...
    LOG_message(LOG_SYSTEM_UNIT_TEST, LOG_SEVERITY_NORMAL, LOG_CHANNEL_ALL, "Total tests: %d - Pass: %d, Fail: %d, Duration: %lu ms",
        total_exec_count,
        total_pass_count,
        total_fail_count,
