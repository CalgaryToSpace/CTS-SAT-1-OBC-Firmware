...
uint8_t TEST_EXEC__HELPER_split_string_by_delimiter()
{
    ...
    char fruit_output[max_string_num][max_string_len];
    for (uint8_t i = 0; i < max_string_num; i++)
    {
        memset(fruit_output[i], 0, sizeof(fruit_output[i]));
    }
    const char *fruits = "apple,banana,cherry,date\0";
    ...
    char empty_output[max_string_num][max_string_len];
    for (uint8_t i = 0; i < max_string_num; i++)
    {
        memset(empty_output[i], 0, sizeof(empty_output[i]));
    }
    const uint8_t num_strings_split_empty = split_string_by_delimiter(empty, 0, ',', empty_output, 0);
    ...
    const char *single = "single\0";
    char single_output[max_string_num][max_string_len];
    for (uint8_t i = 0; i < max_string_num; i++)
    {
    ...
    const char *trailing = "one,two,\0";
    char trailing_output[max_string_num][max_string_len];
    for (uint8_t i = 0; i < max_string_num; i++)
    {
    ...
    const char *leading = ",one,two,three\0";
    char leading_output[max_string_num][max_string_len];
    for (uint8_t i = 0; i < max_string_num; i++)
    {
    ...
    const char *consecutive = "one,,two\0";
    char consecutive_output[max_string_num][max_string_len];
    for (uint8_t i = 0; i < max_string_num; i++)
    {
