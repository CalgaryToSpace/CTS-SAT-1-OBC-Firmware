...
def _send_simple_slow_command(ser: serial.Serial, command: str, *, delay: float = 1.0) -> bytes:
...
def send_file_over_uart(
    ...
    *,
    ...
    with serial.Serial(
        ...
        _send_simple_slow_command(
            # Equivalent on-orbit: TCMD_handle_ax100_tcmds_interval_ms
            ser,
            "CTS1+config_set_int_var(TCMD_handle_umbilical_tcmds_interval_ms,20)!",
        )
            ...
            while True:
                ...
                if incoming_bytes.decode("ascii", errors="ignore").count(expected_str_2) != 1:
                    ...
                    logger.warning(
                        ...
                        "Dev kit may have responded too slowly. Likely not problematic though. "
                        f"Maybe-problematic chunks ({len(problematic_chunk_indexes)}): "
                        f"{problematic_chunk_indexes[-100:]}"
                    )

                chunk_index += 1

                # Validate that the number of ACK (Acknowledgements) matches the number of messages
                # sent.
                # On the actual satellite, the better way to manage this is with the unique uplink
                # message ID timestamps, which are included in each downlink packet.
                ack_count = len(
                    re.findall(r"=\n\d+\n==", all_rx_data.decode("ascii", errors="ignore"))
                )
                if ack_count != chunk_index:
                    logger.warning(
                        f"Running ACK count mismatch. Uplink may have occurred too fast. "
                        "Likely problematic if this message is printed many times consecutively. "
                        f"{ack_count} ACKs for {chunk_index} chunks."
                    )
        ...
        if ack_count != chunk_index:
            logger.error(
                f"Final ACK count mismatch. Uplink may have occurred too fast. "
                f"{ack_count} ACKs for {chunk_index} chunks."
            )
        else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=168,
        help="Chunk size in bytes before base64 encoding (default: 168). Best if divisible by 3 (and by power of 2).",
    )
    parser.add_argument(
        "--delay",
        type=float,
        default=0.3,  # 0.3 is safe and reliable.
        help="Delay in seconds between chunks (default: 0.3)",
    )
