...
static uint8_t normalize_lfs_error(int err) {
    if (err > 0) {
        return 0;
    } else {
        return -err;
...
static int8_t copy_lfs_file_chunk(
    ...
    lfs_file_t out_file;
    if ((err=fw_lfs_file_open(LFS_filesystem_ptr, &out_file, dest_file_path, LFS_O_RDONLY)) != 0) {
        fw_lfs_file_close(LFS_filesystem_ptr, &in_file);
        return err;
    }

    // Get the input file size.
    lfs_soff_t in_file_size = 0;
    if ((err=fw_lfs_file_size(LFS_filesystem_ptr, &in_file)) != 0) {
        fw_lfs_file_close(LFS_filesystem_ptr, &in_file);
        fw_lfs_file_close(LFS_filesystem_ptr, &out_file);
        return err;
    }
    if (byte_count == 0 || (start_offset + byte_count) > in_file_size) {
        byte_count = in_file_size - start_offset;
    }
...
uint8_t blob_main(
    ...
    if (err != 0) {
        FW_SNPRINTF(
            ...
            "copy_file_blob %s %s %d %d failed. LFS error: %d. Runtime: %d ms.",
            arg0_in_path, arg1_out_path, start_offset, byte_count,
            err,
            TIME_uptime_ms() - start_time_ms
        );
        return normalize_lfs_error(err);
    }

    FW_SNPRINTF(
        response_buf, response_buf_len,
        "copy_file_blob %s %s %d %d success in %d ms.",
        arg0_in_path, arg1_out_path, start_offset, byte_count,
        TIME_uptime_ms() - start_time_ms
    );
    return 0;
